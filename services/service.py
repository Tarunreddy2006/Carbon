...
def get_canopy_height(ee_geom):
    ...
    try:
        ...
        final_height = ee.ImageCollection([
            fallback,
            gedi_height
        ]).mosaic()
        ...
        return float(
            stats.getInfo().get("height", 15.0)
        )

    except Exception as e:
        logger.error(
            f"RH95 extraction failed: {e}"
        )
        return 15.0
def analyse_parcel(geojson_geometry):
    ...
    ndmi = s2_median.normalizedDifference(
        ["B8", "B11"]
    ).rename("NDMI")
    ...
    return {
        ...
        "vv": stats.get("VV", -20.0),
        "vh": stats.get("VH", -25.0),
